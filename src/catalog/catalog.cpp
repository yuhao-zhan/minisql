...
CatalogManager::CatalogManager(BufferPoolManager *buffer_pool_manager, LockManager *lock_manager,
    ...
    if (init) {
        ...
        // 2) 在 page 0 上分配并写入
        page_id_t meta_pid;
        Page *page = buffer_pool_manager_->NewPage(meta_pid);
...
dberr_t CatalogManager::CreateTable(const string &table_name, TableSchema *schema, Txn *txn, TableInfo *&table_info) {
    ...
    // 2) 创建表元数据
    TableMetadata *tbl_meta = TableMetadata::Create(table_id, table_name, page_id, schema);
    // 3) 创建表堆
    TableHeap *heap = TableHeap::Create(buffer_pool_manager_, page_id, schema, log_manager_, lock_manager_);
    // 4) 创建表信息
